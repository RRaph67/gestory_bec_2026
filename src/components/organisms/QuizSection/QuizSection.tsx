...
export const QuizSection: React.FC<QuizSectionProps> = ({
  ...
  // Allow each summary to own its complete-material link, with a PDF section fallback.
  const pdfSection = sections.find((s) => s.type === "pdf");
  const summaryPdfUrl = sections.find((s) => s.type === "text")?.url;
  const pdfUrl = summaryPdfUrl || pdfSection?.url;
                      ...
                      {pdfUrl && (
                        <div className="border-t border-slate-200 pt-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                          <p className="text-sm text-slate-500 font-medium">
                            Pelajari materi lengkap dalam format PDF.
                          </p>
                          <a
                            href={pdfUrl}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="inline-flex items-center justify-center gap-2 bg-[#0077B6] text-white px-6 py-3.5 rounded-2xl font-black hover:bg-[#005F92] transition-all text-sm shadow-lg shadow-blue-100"
                          >
